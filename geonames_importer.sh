...
download_geonames_data() {
  ...
  for zip in $zip_codes; do
    wget -c -O "${download_folder}/${zip%.*}_zipcode.zip" http://download.geonames.org/export/zip/$zip;
    unzip -n -o "${download_folder}/${zip%.*}_zipcode.zip" -d "${data_folder}/zipcodes";
  done

  unzip -n -o "${download_folder}/*.zip" -d "${data_folder}"
  rsync -a -v --ignore-existing ${download_folder}/*.txt "${data_folder}"
  echo "Done."
}
